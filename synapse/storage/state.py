...
class StateGroupWorkerStore(EventsWorkerStore, SQLBaseStore):
    ...
    def _get_state_for_groups(self, groups, types=None, filtered_types=None):
        ...
        if missing_groups:
            ...
            self._insert_into_cache(
                ...
                group_to_state_dict, non_member_types, filtered_types,
            )

            self._insert_into_cache(
                self._state_group_members_cache, cache_sequence_m,
                member_state, group_to_state_dict, member_types, None,
            )
    ...
    def _insert_into_cache(self, cache, cache_seq_num, existing_state,
                           group_to_state_dict, types, filtered_types):
        ...
        Args:
            ...
            filtered_types(list[str]|None): Only apply filtering via `types` to this
                ...
                If None, `types` filtering is applied to all events.
        """
        # Reconstruct what we asked from the database.
        if filtered_types or (types and any(k is None for (t, k) in types)):
            types_to_fetch = None
        else:
            types_to_fetch = types
